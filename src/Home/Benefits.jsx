...
import Heading from "../components/subComponents/Heading";
import Section from "../components/subComponents/Section";
import useRazorpayPayment from "../components/useRazorpayPayment"; // Import the custom hook
...
const Benefits = ({ benefits, profile, isAuthenticated, openLoginModal }) => {
  ...
  const handleBuyNow = async (item) => {
    ...
    try {
      // Trigger Razorpay payment with the item's discounted amount, offer ID, and profile
      await handlePayment(
        item.amount - (item.amount * (item.discount / 100)), // Discounted amount
        item.id, // Offer ID
              ...
              >
                ...
                <div className="relative z-10 flex flex-col min-h-[11rem] sm:min-h-[20rem] p-[1.6rem] sm:p-[1.5rem] pb-6 mb-3 rounded-lg bg-opacity-50">
                  ...
                  <div className="flex flex-col mb-2 sm:mb-4">
                    ...
                    <p
                      className="text-sm sm:text-base font-medium text-green-500 text-center"
                      style={{ color: "#FFDE59" }}
                    >
                      Discount: {item.discount}%
                    </p>
                    <p
                      className="text-sm sm:text-base font-medium text-center line-through"
                      style={{ color: "#E6AFFC" }}
                    >
                      ₹{item.amount}
                    </p>
